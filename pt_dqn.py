...
env = gym.make('BalloonLearningEnvironment-v0') #"CartPole-v1")
...
class DQN(nn.Module):
    ...
    def __init__(self, n_observations, n_actions):
        ...
        self.hlayers = torch.nn.ModuleList([nn.Linear(LAYER_SIZE, LAYER_SIZE) for _ in range(N_LAYERS-2)])
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            return model.policy_net(state).max(1).indices.view(1, 1)
    else:
...
def optimize_model():
    ...
    with torch.no_grad():
        next_state_values[non_final_mask] = model.target_net(non_final_next_states).max(1).values
    ...
    torch.nn.utils.clip_grad_value_(model.policy_net.parameters(), 100)
    optimizer.step()
