...
class BasePolicy:
    ...
    def act(self, observations, return_log_prob = False):
        ...
        distro = self.action_distribution(observations)
        sampled_actions = distro.sample()
    
        if return_log_prob:
            log_probs = distro.log_prob(sampled_actions).detach().cpu().numpy()
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = torch.distributions.MultivariateNormal(
            loc=loc,
            covariance_matrix=torch.diag_embed(self.std()).unsqueeze(0).expand(B,-1,-1)
        )
