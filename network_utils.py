...
#
# The original network_utils.py from CS234 HW2.
# Used for utilities such as build_mlp.
#
...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    for _ in range(n_layers-1):
        layers += [nn.ReLU(), nn.Linear(size, size)]
    layers += [nn.ReLU(), nn.Linear(size, output_size)]
