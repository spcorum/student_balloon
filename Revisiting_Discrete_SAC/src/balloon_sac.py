...
#
# This file is modified from atari_sac.py to suit the balloon learning environment
#
# See the original Revisiting repo: https://github.com/coldsummerday/Revisiting-Discrete-SAC
#
...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="BalloonLearningEnvironment-v0")
    ...
    parser.add_argument("--step-per-epoch", type=int, default=960)
    parser.add_argument("--step-per-collect", type=int, default=960)
    ...
    parser.add_argument('--invert-reward', action='store_true')
...
class EnvWrapper(gym.Env):
    def __init__(self, invert_reward):
        self.env = gym.make('BalloonLearningEnvironment-v0')
...
def invert_reward_step(self, action):
    state, reward, done, info = self.env.step(action)
    reward *= -1.0
    return state, reward, done, info

def make_balloon_env(seed, invert_reward):
    env = gym.make('BalloonLearningEnvironment-v0')
    if invert_reward:
        env.step = types.MethodType(invert_reward_step, env)
    train_envs = env
    #ShmemVectorEnv(
    #    [
    #        lambda:
    #        gym.make('BalloonLearningEnvironment-v0')
    #        for _ in range(1)
    #    ]
    #)
    test_envs = env
...
def test_discrete_sac(args=get_args()):
    ...
    #)
    env, train_envs, test_envs = make_balloon_env(
        args.seed, args.invert_reward
    )
    ...
    class Net(nn.Module):
        ...
        def forward(self, x, state):    # needs to be called "state" (used as kwarg)
            x = torch.as_tensor(x, device=args.device, dtype=torch.float32)
    ...
    print(args.state_shape, type(args.state_shape))
    print(args.action_shape, type(args.action_shape))

    net = Net()
    actor = Actor(net, args.action_shape, device=args.device, softmax_output=False,
                  preprocess_net_output_dim=args.action_shape)
    ...
    buffer = VectorReplayBuffer(
        args.buffer_size,
        1,#buffer_num=len(train_envs),
        ignore_obs_next=False,
        save_only_last_obs=False,
        stack_num=1,#args.frames_stack,
        save_entropy = True,
    )
    # collector
